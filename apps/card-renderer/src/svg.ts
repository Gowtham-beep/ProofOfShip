...
export function generateCard(data: CardData): string {
  ...
  const suffixWidth = getScoreSuffixWidth(data.score);
  const complexityBadgeWidth = data.complexityTier.length * 7 + 16;
  ...
  const barsSvg = bars.map(({ key, value }) => {
    ...
    <text x="458" y="${y + 8}" font-family="system-ui, -apple-system, sans-serif" font-size="10" font-weight="400" fill="#8b949e" text-anchor="end">${value}</text>`;
  }).join('');
  ...
  <text x="95" y="90" font-family="system-ui, -apple-system, sans-serif" font-size="32" font-weight="700" fill="${scoreColor}">${data.score}</text>
  ...
  <text x="${95 + complexityBadgeWidth / 2}" y="120" font-family="system-ui, -apple-system, sans-serif" font-size="10" font-weight="400" fill="#8b949e" text-anchor="middle">${escapeXml(data.complexityTier)}</text>
  ...
  <text x="15" y="188" font-family="system-ui, -apple-system, sans-serif" font-size="10" font-weight="400" fill="#8b949e">${data.totalRepos} repos analyzed</text>
